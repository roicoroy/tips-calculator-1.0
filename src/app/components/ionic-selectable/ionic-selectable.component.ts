...
export class IonicSelectableComponent implements ControlValueAccessor, OnInit, DoCheck {
  ...
  private _label: string | any = null;
   ...
   */
  @Input()
  modalEnterAnimation: AnimationBuilder | any = null;
   ...
   */
  @Input()
  modalLeaveAnimation: AnimationBuilder | any = null;
 ...
 */
   ...
   */
  @Input()
  itemValueField: string | any = null;
   ...
   */
  @Input()
  groupValueField: string | any = null;
...
*/
  @Input()
  groupTextField: string | any = null;
    ...
    if (this._hasOnSearch()) {
      ...
      if (!this._searchText || !this._searchText.trim()) {
        ...
        this._groups.forEach(group => {
          const items = group.items.filter((item: { [x: string]: any; }) => {
  ...
  _isItemDisabled(item: any): boolean | any {
  ...
  private _areGroupsEmpty(groups: any[]) {
    return groups.length === 0 || groups.every((group: { items: string | any[]; }) => {
 ...
 */
  deleteItem(item: any): Promise<void> {
    ...
    if (this.value) {
      if (this.isMultiple) {
        const values = this.value.filter((value: { id: any; }) => {
